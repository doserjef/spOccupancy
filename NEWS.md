...
spOccupancy v0.7.0 contains a variety of substantial updates, most notability functionality for fitting non-spatial and spatial multi-species multi-season occupancy models, as well as multi-species spatially-varying coefficient models. There are also a variety of smaller bug fixes/additional error handling that will help eliminate some common hard-to-interpret errors that users encountered.

+ New functionality for fitting multi-species, multi-season occupancy models. The function `tMsPGOcc()` fits non-spatial, multi-season, multi-species occupancy models, and the function `sfTMsPGOcc()` fits spatial, multi-season occupancy models. The spatially-explicit function also inherently account for species correlations with a spatial factor modeling approach (e.g., they are joint species distribution models with imperfect detection and species correlations). See [Doser et al. 2023](https://esajournals.onlinelibrary.wiley.com/doi/10.1002/ecy.4137#pane-pcw-related) for statistical details on the spatial factor modeling approach. A short vignette is available on describing how to run a non-spatial and spatial multi-species, multi-season occupancy model. 
+ New functionality for multi-species spatially-varying coefficient occupancy models for single-season (`svcMsPGOcc()`) and multi-season (`svcTMsPGOcc()`) models. These approaches use a spatial factor modeling approach for each of the SVCs to make the models relatively computationally efficient. These functions inherently account for species correlations in this approach. There is not currently a vignette for these functions, but the arguments and data formatting for the functions follows almost exactly the formatting for multi-species spatial factor models (`sfMsPGOcc()`) and multi-species, multi-season spatial models (`sfTMsPGOcc()`), so the corresponding vignetes for those functions should help. There is of course an example with simulated data in the manual pages for the functions as well. Statistical details for the single-season model are available in [this preprint](https://www.jeffdoser.com/). 
