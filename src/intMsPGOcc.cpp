...
extern "C" {
  SEXP intMsPGOcc(SEXP y_r, SEXP X_r, SEXP Xp_r, SEXP XRE_r, 
    ...
    SEXP alphaSamples_r; 
    PROTECT(alphaSamples_r = allocMatrix(REALSXP, nAlpha, nPost)); nProtect++;
    SEXP zSamples_r; 
    ...
    for (s = 0; s < nSamples; s++) {
      ...
      if(info != 0){error("c++ error: dpotrf AAlphaComm1 failed\n");}
      ...
      F77_NAME(dpotrf)(lower, &pDet, tmp_ppDet, &pDet, &info FCONE); 
      if(info != 0){error("c++ error: dpotrf AAlphaComm2 failed\n");}
