...
#include <omp.h>
#endif
void updateBF1Ms(double *B, double *F, double *c, double *C, double *coords, int *nnIndx, int *nnIndxLU, int n, int m, double sigmaSq, double phi, double nu, int covModel, double *bk, double nuUnifb){
...
extern "C" {
  SEXP spMsPGOccNNGP(SEXP y_r, SEXP X_r, SEXP Xp_r, SEXP coords_r, SEXP pocc_r, SEXP pdet_r, 
     ...
     * Initial constants
     * *******************************************************************/
    int i, j, k, s, g, t, q, r, info, nProtect=0, ii;    
    ...
    // Spatial random effects
    SEXP wSamples_r; 
    PROTECT(wSamples_r = allocMatrix(REALSXP, JN, nSamples)); nProtect++; 
    ...
    SEXP thetaSamples_r; 
    PROTECT(thetaSamples_r = allocMatrix(REALSXP, nThetaN, nSamples)); nProtect++; 
    // For NNGP
    ...
    for (i = 0; i < N; i++) {
    updateBF1Ms(&B[i * nIndx], &F[i*J], &c[i * m*nThreads], &C[i * mm * nThreads], coords, nnIndx, nnIndxLU, J, m, theta[sigmaSqIndx * N + i], theta[phiIndx * N + i], nu[i], covModel, &bk[i * sizeBK], nuB[0]);
    }
...
#pragma omp parallel for private (e, ii, b) reduction(+:a, logDet)
	  ...
	  theta[sigmaSqIndx * N + i] = rigamma(sigmaSqA[i] + J / 2.0, sigmaSqB[i] + 0.5 * a * theta[sigmaSqIndx * N + i]); 
	    ...
	    nu[i] = theta[nuIndx * N + i];
          ...
          updateBF1Ms(&B[i * nIndx], &F[i*J], &c[i * m*nThreads], &C[i * mm * nThreads], coords, nnIndx, nnIndxLU, J, m, theta[sigmaSqIndx * N + i], theta[phiIndx * N + i], nu[i], covModel, &bk[i * sizeBK], nuB[i]);
          a = 0;
          logDet = 0;
...
#pragma omp parallel for private (e, ii, b) reduction(+:a, logDet)
          ...
          updateBF1Ms(BCand, FCand, &c[i * m*nThreads], &C[i * mm * nThreads], coords, nnIndx, nnIndxLU, J, m, theta[sigmaSqIndx * N + i], phiCand, nuCand, covModel, &bk[i * sizeBK], nuB[i]);
...
#pragma omp parallel for private (e, ii, b) reduction(+:a, logDet)
         ...
         *Save samples
         *******************************************************************/
        F77_NAME(dcopy)(&pOcc, betaComm, &inc, &REAL(betaCommSamples_r)[g*pOcc], &inc);
        F77_NAME(dcopy)(&pDet, alphaComm, &inc, &REAL(alphaCommSamples_r)[g*pDet], &inc);
        F77_NAME(dcopy)(&pOcc, tauBeta, &inc, &REAL(tauBetaSamples_r)[g*pOcc], &inc);
        F77_NAME(dcopy)(&pDet, tauAlpha, &inc, &REAL(tauAlphaSamples_r)[g*pDet], &inc);
        F77_NAME(dcopy)(&pOccN, beta, &inc, &REAL(betaSamples_r)[g*pOccN], &inc); 
        F77_NAME(dcopy)(&pDetN, alpha, &inc, &REAL(alphaSamples_r)[g*pDetN], &inc); 
        F77_NAME(dcopy)(&JN, psi, &inc, &REAL(psiSamples_r)[g*JN], &inc); 
        F77_NAME(dcopy)(&JN, z, &inc, &REAL(zSamples_r)[g*JN], &inc); 
        F77_NAME(dcopy)(&JN, w, &inc, &REAL(wSamples_r)[g*JN], &inc); 
        F77_NAME(dcopy)(&nThetaN, theta, &inc, &REAL(thetaSamples_r)[g*nThetaN], &inc); 
    ...
    SET_VECTOR_ELT(result_r, 9, thetaSamples_r); 
    ...
    SET_VECTOR_ELT(resultName_r, 9, mkChar("theta.samples")); 
