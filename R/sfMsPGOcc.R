...
sfMsPGOcc <- function(occ.formula, det.formula, data, inits, priors, 
  ...
  if (!NNGP) {
    ...
    for (i in 1:n.chains) {
      ...
      out.tmp[[i]] <- .Call("sfMsPGOccNNGP", y, X, X.p, coords, X.p.re, 
			    p.occ, p.det, p.det.re, J, n.obs, K, N, n.det.re, n.det.re.long,
        	            q, n.neighbors, nn.indx, nn.indx.lu, u.indx, u.indx.lu, ui.indx,
        	            beta.inits, alpha.inits, z.inits,
        	            beta.comm.inits, 
        	            alpha.comm.inits, tau.sq.beta.inits, 
        	            tau.sq.alpha.inits, phi.inits, 
        	            lambda.inits, nu.inits, sigma.sq.p.inits, 
			    alpha.star.inits, z.long.indx, 
	...
	X.p.re.0 <- X.p.re[!y.indx, , drop = FALSE]
      ...
      out.fit <- .Call("sfMsPGOccNNGP", y.fit, X.fit, X.p.fit, coords.fit, X.p.re.fit, 
		       p.occ, p.det, p.det.re, J.fit, n.obs.fit, K.fit, 
		       N, n.det.re, n.det.re.long,
        	       q, n.neighbors, nn.indx.fit, nn.indx.lu.fit, u.indx.fit, 
		       u.indx.lu.fit, ui.indx.fit,
        	       ...
        	       tau.sq.alpha.inits, phi.inits, 
        	       lambda.inits, nu.inits, sigma.sq.p.inits, 
		       alpha.star.inits, z.long.indx.fit, 
		       alpha.star.indx, mu.beta.comm, 
        	       mu.alpha.comm, Sigma.beta.comm, Sigma.alpha.comm, 
        	       tau.sq.beta.a, tau.sq.beta.b, tau.sq.alpha.a, 
        	       tau.sq.alpha.b, phi.a, phi.b,
