    ...
    if (!k.fold.only) {
      if (parallel.chains) {
        ...
        out.tmp <- foreach(i = 1:n.chains) %dopar% {
          .Call("svcPGOccNNGP", y, X, X.w, X.p, coords, X.re, X.p.re, consts, 
                ...
                w.inits, phi.inits.list[[i]], sigma.sq.inits.list[[i]], nu.inits.list[[i]], 
                z.long.indx, beta.star.indx, beta.level.indx, alpha.star.indx, 
