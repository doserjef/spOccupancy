  ...
  if (!NNGP) {
    ...
    if (parallel.chains) {
      ...
      out.tmp <- foreach(i = 1:n.chains) %dorng% {
        .Call("svcTMsPGOccNNGP", y, X.big, X.w.big, X.p, 
              ...
              lambda.inits.list[[i]], nu.inits.list[[i]], 
              sigma.sq.psi.inits.list[[i]], sigma.sq.p.inits.list[[i]], 
              beta.star.inits.list[[i]], alpha.star.inits.list[[i]], z.long.indx, z.year.indx, 
              ...
              samples.info, chain.info, ar1.vals.list[[i]], svc.by.sp, var.lambda, 
              grid.index.c)
      }
