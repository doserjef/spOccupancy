  ...
  if (!NNGP) {
      ...
      if (parallel.chains) {
        ...
        for (i in 2:n.chains) {
          if ((!fix.inits)) {
            ...
            if (!fixed.sigma.sq) {
              if (sigma.sq.ig) {
                sigma.sq.inits.list[[i]] <- rigamma(N, sigma.sq.a, sigma.sq.b)
              } else {
                sigma.sq.inits.list[[i]] <- runif(N, sigma.sq.a, sigma.sq.b)
              }
            }
            phi.inits.list[[i]] <- runif(N, phi.a, phi.b)
            if (cov.model == 'matern') {
              nu.inits.list[[i]] <- runif(N, nu.a, nu.b)
            }
          }
        }
        ...
        out.tmp <- foreach(i = 1:n.chains) %dorng% {
          .Call("spMsPGOcc", y, X, X.p, coords.D, X.re, X.p.re, consts, 
          	    K, n.occ.re.long, n.det.re.long, 
                beta.inits.list[[i]], alpha.inits.list[[i]], z.inits,
                beta.comm.inits.list[[i]], alpha.comm.inits.list[[i]], tau.sq.beta.inits.list[[i]], 
                tau.sq.alpha.inits.list[[i]], w.inits, phi.inits.list[[i]], 
                sigma.sq.inits.list[[i]], nu.inits.list[[i]], 
    ...
    if (!k.fold.only) {
      if (parallel.chains) {
        ...
        registerDoParallel(par.cl)
        out.tmp <- foreach(i = 1:n.chains) %dorng% {
          .Call("spMsPGOccNNGP", y, X, X.p, coords, X.re, X.p.re, consts, 
          	    K, n.occ.re.long, n.det.re.long, 
