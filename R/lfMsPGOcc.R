# Latent factor model for multi-species Polya-Gamma occupancy model. 
lfMsPGOcc <- function(occ.formula, det.formula, data, inits, priors, 
  ...
  for (i in 1:n.chains) {
    ...
    storage.mode(curr.chain) <- "integer"
    # Run the model in C
    out.tmp[[i]] <- .Call("lfMsPGOcc", y, X, X.p, X.p.re,  
			  p.occ, p.det, p.det.re, J, n.obs, K, N, n.det.re, 
			  n.det.re.long, q, beta.inits, alpha.inits, 
			  z.inits, beta.comm.inits, 
      	                  alpha.comm.inits, tau.sq.beta.inits, 
      	                  tau.sq.alpha.inits, lambda.inits, sigma.sq.p.inits, 
			  alpha.star.inits, z.long.indx, alpha.star.indx, mu.beta.comm, 
      	                  ...
      	                  tau.sq.alpha.b, sigma.sq.p.a, sigma.sq.p.b, 
  ...
  if (!missing(k.fold)) {
    ...
    model.deviance <- foreach (i = 1:k.fold, .combine = "+") %dopar% {
      ...
      out.fit <- .Call("lfMsPGOcc", y.fit, X.fit, X.p.fit, X.p.re.fit, 
      		 p.occ, p.det, p.det.re, 
      		 J.fit, n.obs.fit, K.fit, N, n.det.re, n.det.re.long, q,
      	         ...
      	         mu.alpha.comm, Sigma.beta.comm, Sigma.alpha.comm, 
      	         tau.sq.beta.a, tau.sq.beta.b, tau.sq.alpha.a, 
      	         tau.sq.alpha.b, sigma.sq.p.a, sigma.sq.p.b, n.samples, 
      ...
      out.pred <- predict.lfMsPGOcc(out.fit, X.0, verbose = FALSE)
