  ...
  if (!k.fold.only) {
    if (parallel.chains) {
      ...
      out.tmp <- foreach(i = 1:n.chains) %dorng% {
        .Call("lfMsPGOcc", y, X, X.p, X.re, X.p.re, consts,
              K, n.occ.re.long, n.det.re.long, beta.inits.list[[i]], 
              alpha.inits.list[[i]], 
              z.inits, beta.comm.inits.list[[i]], 
          	  alpha.comm.inits.list[[i]], tau.sq.beta.inits.list[[i]], 
          	  tau.sq.alpha.inits.list[[i]], lambda.inits.list[[i]], 
