svcMsPGOcc <- function(occ.formula, det.formula, data, inits, priors, 
  ...
  if (!NNGP) {
    ...
    if (parallel.chains) {
      ...
      alpha.comm.inits.list <- list()
      tau.sq.beta.inits.list <- list()
      tau.sq.alpha.inits.list <- list()
      ...
      for (i in 1:n.chains) {
        ...
        alpha.comm.inits.list[[i]] <- alpha.comm.inits
        tau.sq.beta.inits.list[[i]] <- tau.sq.beta.inits
        tau.sq.alpha.inits.list[[i]] <- tau.sq.alpha.inits
      ...
      for (i in 2:n.chains) {
        if ((!fix.inits)) {
          ...
          lambda.inits.list[[i]] <- list()
          for (j in 1:p.svc) {
            lambda.inits.list[[i]][[j]] <- matrix(0, N, q)
            diag(lambda.inits.list[[i]][[j]]) <- 1
            lambda.inits.list[[i]][[j]][lower.tri(lambda.inits.list[[i]][[j]])] <- rnorm(sum(lower.tri(lambda.inits.list[[i]][[j]])))
          }
          lambda.inits.list[[i]] <- unlist(lambda.inits.list[[i]])
          phi.inits.list[[i]] <- runif(q.p.svc, phi.a, phi.b)
          if (cov.model == 'matern') {
            nu.inits.list[[i]] <- runif(q.p.svc, nu.a, nu.b)
          }
        }
      }
      ...
      out.tmp <- foreach(i = 1:n.chains) %dorng% {
        .Call("svcMsPGOccNNGP", y, X.big, X.w.big, X.p, coords, X.re, X.p.re, 
              range.ind, consts, K, n.occ.re.long, n.det.re.long, 
              n.neighbors, nn.indx, nn.indx.lu, u.indx, u.indx.lu, ui.indx,
              beta.inits.list[[i]], alpha.inits.list[[i]], z.inits,
              beta.comm.inits.list[[i]], 
              alpha.comm.inits.list[[i]], tau.sq.beta.inits.list[[i]], 
              tau.sq.alpha.inits.list[[i]], phi.inits.list[[i]], 
              lambda.inits.list[[i]], w.inits, nu.inits, 
              sigma.sq.psi.inits.list[[i]], sigma.sq.p.inits.list[[i]], 
              beta.star.inits.list[[i]], alpha.star.inits.list[[i]], z.long.indx, 
