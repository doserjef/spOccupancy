...
where $p(\cdot)$ denotes a probability density function. The NNGP prior achieves computational efficiency by replacing the conditioning sets on the right-hand side of (\ref{wConditional}) with a set of new conditioning sets, whose maximum size is determined by a pre-specified number of neighbors, $m$, where $m << J$. [@datta2016hierarchical] showed that $m = 15$ provides nearly identical inference to the full GP under a variety of scenarios. Let $N(\bm{s}_j)$ denote the set of at most $m$ neighbors for location $\bm{s}_j$. Following [@vecchia1988estimation], we set $N(\bm{s}_j)$ to be the set of at most $m$ nearest neighbors of $\bm{s}_j$ from $\{\bm{s}_1, \bm{s}_2, \dots, \bm{s}_{j - 1}\}$ with respect to Euclidean distance. Note that this requires the set of $\mathcal{L}$ locations to have some prespecified ordering, where here we order the coordinates along the horizontal axis. Through careful construction of the neighbor sets and set of spatial locations as a directed acyclic graph, Gaussian distribution theory reveals the NNGP prior yields a new joint density for $\textbf{w}_h$, denoted $\tilde{p}(\textbf{w}_h)$. Let $\textbf{w}_h(N(\bm{s}_j))$ denote the at most $m$ realizations of the $t^{\text{th}}$ NNGP at the locations in the neighbor set $N(\bm{s}_j)$. Let $C(\cdot, \bm{\theta}_h)$ denote the covariance function of the original Gaussian Process (GP) from which the $t^{\text{th}}$ NNGP is derived. For any two sets $A_1$ and $A_2$, define $\text{C}_{A_1, A_2}(\bm{\theta}_h)$ as the covariance matrix between the observations in $A_1$ and $A_2$ for the $t^{\text{th}}$ GP. Our NNGP prior for $\textbf{w}_h$ thus takes the form
\begin{equation}\label{NNGP}
     \tilde{p}(\textbf{w}_h) = \prod_{j = 1}^J \text{Normal}(\text{w}_h(\bm{s}_j) \mid \textbf{w}_h(N(\bm{s}_j))\textbf{b}_h(\bm{s}_j), \text{f}_h(\bm{s}_j)),
\end{equation}
...
\end{equation}
with $\textbf{b}_h(\bm{s}_1) = \bm{0}$ for all $h = 1, \dots, \tilde{H}$. Further, we have
...
\end{equation}
where $\text{f}_h(\bm{s}_1) = 0$ for all $h = 1, \dots, \tilde{H}$.
...
where $\textbf{w}(N(\bm{s}_j))$ is a stacked $mq \times 1$ vector of the $m$ realizations of each of the $t$ NNGPs at the locations in $N(\bm{s}_j)$ and $\bm{S}_{j, \beta}$ is a $\tilde{H} \times \tilde{H}$ diagonal matrix with each diagonal entry equal to the the \pg auxiliary variable at site $j$.
...
\propto p(\phi_h) \times \prod_{j = 1}^J N\left(\text{w}_h(\bm{s}_j) \mid \textbf{b}_h(\bm{s}_j)^\top\textbf{w}_h (N(\bm{s}_j)), f_h (\bm{s}_j). \right)
\end{array}
\end{equation}

The same update is used for the spatial smoothness parameter $\nu_h$ if using a Mat\'ern correlation function.
...
\sigma^2_h \mid \cdot \sim IG(a_\sigma+J/2,\; b_\sigma+\frac 12 \sum_{j=1}^J \left(\text{w}_j(\bm{s}_j)-\textbf{b}_h(\bm{s}_j)'\textbf{w}_h(N(\bm{s}_j)) \right) ^2/f^*_h(\bm{s}_j).
\end{equation}
...
\begin{equation}\label{psiMS}
   \text{logit}(\psi_i(\bm{s}_j)) = (\beta_{i, 1} + \delta_1\text{w}^\ast_{i, 1}(\bm{s}_j)) + \sum_{h = 2}^H\text{x}_h(\bm{s}_j)\{\beta_{i, t} + \delta_j\text{w}^\ast_{i, t}(\bm{s}_j)\},
\end{equation}

with all parameters as defined before, but now spatial and non-spatial effects are unique to each species. Note we assume the same variables have spatially-varying effects across species (i.e., $\delta_{i, t} = \delta_h$ for all $i$), although this could be modified to allow $\delta_h$ to vary by species by either setting values \textit{a priori} or estimating the indicator variables within the model itself. We model the non-spatial component of the $t$th regression coefficient for each species $i$ hierarchically from a common community-level distribution to share information across species [@dorazio2005; @gelfand2005modelling]. More specifically, we have

\begin{equation}\label{betaMS}
     \beta_{i, t} \sim \text{Normal}(\mu_{\beta_h}, \tau^2_{\beta_h}),
\end{equation}
...
\end{equation}
where $\bm{\lambda}_{i, t}^\top$ is the $i$th row of factor loadings from the $N \times q$ loadings matrix $\bm{\Lambda}_h$, and $\textbf{w}_h(\bm{s}_j)$ is a $q \times 1$ vector of independent spatial factors at site $j$. As in the single-species SVC occupancy model, we model each of the $r = 1, \dots, q$ spatial factors for each of the $\tilde{H}$ spatially-varying effects with an NNGP prior following
...
Let $\tau^2_{t, \beta}$ denote the community-level variance for the $t^{\text{th}}$ occurrence parameter ($h = 1, \dots, p_{\psi}$). We assign an inverse gamma prior to $\tau^2_{t, \beta}$ with shape parameter $a_{\tau_{t, \beta}}$ and scale parameter $b_{\tau_{t, \beta}}$. Our full conditional is then

\begin{equation}
\tau^2_{t, \beta} \mid \cdot \sim \text{IG}(a_{\tau_{t, \beta}} + \frac{N}{2}, b_{\tau_{t, \beta}} + \frac{\sum_{i = 1}^N(\beta_{i, t} - \mu_{\beta_h})^2}{2}).
(\#eq:tauBeta)
\end{equation}
...
Let $\tau^2_{t, \alpha}$ denote the community-level variance for the $t^{\text{th}}$ detection parameter ($h = 1, \dots, p_{H}$). We assign an inverse gamma prior to $\tau^2_{t, \alpha}$ with shape parameter $a_{\tau_{t, \alpha}}$ and scale parameter $b_{\tau_{t, \alpha}}$. Our full conditional is then

\begin{equation}
\tau^2_{t, \alpha} \mid \cdot \sim \text{IG}(a_{\tau_{t, \alpha}} + \frac{N}{2}, b_{\tau_{t, \alpha}} + \frac{\sum_{i = 1}^N(\alpha_{i, t} - \mu_{\alpha_h})^2}{2}).
(\#eq:tauAlpha)
\end{equation}
...
\begin{equation}
\bm{\mu}^\ast_{\beta, i} = \bm{X}^{\top}\bm{Z}^\ast_i(\bm{z}_i - 0.5 \bm{1}_J - \bm{S}_{\beta}\sum_{h = 1}^{\tilde{p}_\psi}\tilde{\bm{X}}_{t, J}\bm{Z}^\ast_i\textbf{W}_h^\top\bm{\lambda}_{i, t}) + \bm{T}_{\beta}^{-1}\bm{\mu_{\beta}},
\end{equation}
...
where $\textbf{b}_{r, t}(\bm{s}_1) = \bm{0}$ for all $r = 1, \dots, q$ and $h = 1, \dots, \tilde{H}$. Further, we have
...
where $f_{r, t}(\bm{s}_1) = 0$ for all $r = 1, \dots, q$ and $h = 1, \dots, \tilde{H}$. For any two locations $\bm{s}_1$ and $\bm{s}_2$, if $\bm{s}_1 \in N(\bm{s}_2)$ and is the $l^{\text{th}}$ member of $N(\bm{s}_2)$, then define $b_{r, t}(\bm{s}_2, \bm{s}_1)$ as the $l^{\text{th}}$ entry of $\textbf{b}_{r, t}(\bm{s}_2)$. Let $U(\bm{s}_1) = \{\bm{s}_2 \in S \mid \bm{s}_1 \in N(\bm{s}_2)\}$ be the collection of locations $\bm{s}_2$ for which $\bm{s}_1$ is a neighbor, where $S$ is the set of all $J$ spatial locations. For every $\bm{s}_2 \in U(\bm{s}_1)$, define $a_{r, t}(\bm{s}_2, \bm{s}_1) = \text{w}_{r, t}(\bm{s}_2) - \sum_{\bm{s} \in N(\bm{s}_2), \bm{s} \neq \bm{s}_2} \text{w}_{r, t}(\bm{s})b_{r, t}(\bm{s}_2, \bm{s})$. Extending this to matrix notation, let $\bm{B}_h(\bm{s}_j)$ be a $q \times mq$ block matrix, with each $q \times q$ diagonal block containing the elements of $\bm{b}_{r, t}(\bm{s}_j)$ for each of the $r = 1, \dots q$ spatial factors for the $h = 1, \dots, \tilde{H}$ spatially-varying coefficients for each of the specific $m$ neighbors. Let $\bm{F}_h(\bm{s}_j)$ be a $q \times q$ diagonal matrix with diagonal elements of $f_{r, t}(\bm{s}_j)$. Let $\bm{a}_h(\bm{s}, \bm{s}_j)$ contain the values $a_{r, t}(\bm{s}, \bm{s}_j)$ for each of the $r = 1, \dots, q$ latent factors for the $t$th spatially-varying coefficient. Using this notation, the full conditional distribution for $\textbf{w}_h(\bm{s}_j)$ is

\begin{equation}
  \begin{array}{c}
    \textbf{w}_h(\bm{s}_j) \mid \cdot \sim N_q(\bm{\mu}_{j, t}\bm{\Sigma}_{j, t}, \bm{\Sigma}_{j, t}) \mbox{ where, }\\
    \bm{\mu}_{j, t} = \bm{F}_h(\bm{s}_j)^{-1}\bm{B}_h(\bm{s}_j)\textbf{w}_h(N(\bm{s}_j)) + \sum_{\bm{s} \in U(\bm{s}_j)}\bm{B}_h(\bm{s}, \bm{s}_j)^\top\bm{F}_h(\bm{s}_j)^{-1}\bm{a}_h(\bm{s}, \bm{s}_j) + \\
    \bm{\Lambda}_h^\top\tilde{\bm{X}}_{t, N}(\bm{s}_j)\bm{Z}^\ast_{N}(\bm{s}_j)\bm{S}_{N}(\bm{s}_j)(\bm{S}_{N}(\bm{s}_j)^{-1}(\bm{z}(\bm{s}_j) - 0.5\bm{1}_N) - \bm{Z}^\ast_N(\bm{s}_j)(\bm{X}(\bm{s}_j)^\top\bm{\beta} + \sum_{k \neq t}^{\tilde{p}}\tilde{\bm{X}}_{k, N}(\bm{s}_j)\bm{\Lambda}_k\textbf{w}_k(\bm{s}_j))) \mbox{ and } \\
    \bm{\Sigma}_{j, t} = (\bm{F}_h(\bm{s}_j)^{-1} + \sum_{\bm{s} \in U(\bm{s}_j)} \bm{B}_h(\bm{s}, \bm{s}_j)^\top\bm{F}_h(\bm{s}_j)^{-1}\bm{B}_h(\bm{s}, \bm{s}_j) + \\
    (\bm{\Lambda}_h^\top\tilde{\bm{X}}_{t, N}(\bm{s}_j)\bm{Z}^\ast_n(\bm{s}_j))\bm{S}_{N}(\bm{s}_j)(\bm{\Lambda}_h^\top\tilde{\bm{X}}_{t, N}(\bm{s}_j)\bm{Z}^\ast_n(\bm{s}_j))^\top)^{-1},
  \end{array}
(\#eq:w)
\end{equation}
...
where $\dot{\textbf{w}}_{t, i}$ is the $i^{\text{th}}$ column of $\textbf{W}_h$, and all other variables are as defined previously.

### Update spatial range parameters ($\bm{\phi}$)

We use a Metropolis within Gibbs step to sample $\bm{\phi}$. The full conditional posterior density for $\phi_{r, t}$ for each $r = 1, \dots, q$ and $h = 1, \dots, \tilde{H}$ is proportional to
\begin{equation}
\begin{array}{c}
