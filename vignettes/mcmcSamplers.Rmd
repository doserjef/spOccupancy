...
&\text{logit}(p_{j, k}) = \bm{v}'_{j, k}\bm{\alpha},
\end{split}
(\#eq:yPGOcc)
\end{equation}
...
& \text{Normal}(\bm{\beta} \mid \bm{\mu}_{\beta}, \bm{\Sigma}_{\beta}) \times \\
& \text{Normal}(\bm{\alpha} \mid \bm{\mu}_{\alpha}, \bm{\Sigma}_{\alpha}) \\
\end{aligned}
$$
...
\begin{equation}
\bm{\beta} \mid \cdot \sim \text{Normal}\Big([\bm{\Sigma}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}[\bm{X}'(\bm{z} - 0.5 \bm{1}_J) + \bm{\Sigma}_{\beta}^{-1}\bm{\mu}_{\beta}], [\bm{\Sigma}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}\Big), 
(\#eq:betaPostPGOcc)
\end{equation}
...
\begin{equation}
\bm{\alpha} \mid \cdot \sim \text{Normal}\Big([\bm{\Sigma}_{\alpha}^{-1} + \bm{\tilde{V}}'\bm{S}_{\alpha}\bm{\tilde{V}}]^{-1}[\bm{\tilde{V}}'(\bm{\tilde{y}} - 0.5 \bm{1}_{J^*}) + \bm{\Sigma}_{\alpha}^{-1}\bm{\mu}_{\alpha}], [\bm{\Sigma}_{\alpha}^{-1} + \bm{\tilde{V}}'\bm{S}_{\alpha}\bm{\tilde{V}}]^{-1}\Big).
(\#eq:alphaPostPGOcc)
\end{equation}
...
\begin{equation}
\begin{split}
&\text{logit}(\psi_{j}^{(q)}) = \bm{x}_{0, j}' \bm{\beta}^{(q)}, \\
...
\begin{equation}
\text{logit}(\psi_j) = \bm{x}'_j \bm{\beta} + \text{w}_j, 
\end{equation}

where w$_j$ is a realization from a zero-mean spatial Gaussian Process, i.e., 

\begin{equation}
\text{\textbf{w}} \sim \text{Normal}(\bm{0}, \bm{\Sigma}(\bm{s}, \bm{s}', \bm{\theta})).
(\#eq:fullGP)
\end{equation}
...
$$
\begin{aligned}
\lbrack \bm{\alpha}, \bm{\beta}, \bm{z}, \bm{\omega}_{\beta}, \bm{\omega}_{\alpha}, \text{\textbf{w}}, \bm{\theta} \mid \bm{Y} \rbrack \propto \prod_{j = 1}^J \prod_{k = 1}^{K_j} &\text{Bernoulli}(y_{j, k} \mid p_{j, k} z_j) \times \\
& \text{Bernoulli}(z_j \mid \psi_j) \times \\
& \text{Normal}(\text{\textbf{w}} \mid \bm{0}, \bm{\Sigma}(\bm{s}, \bm{s}', \bm{\theta})) \times \\
& \text{PG}(\omega_{j, \beta} \mid 1, 0) \times \\
& \text{PG}(\omega_{j, k, \alpha} \mid 1, 0) \times \\
& \text{Normal}(\bm{\beta} \mid \bm{\mu}_{\beta}, \bm{\Sigma}_{\beta}) \times \\
& \text{Normal}(\bm{\alpha} \mid \bm{\mu}_{\alpha}, \bm{\Sigma}_{\alpha}) \times \\
...
\begin{equation}
\bm{\beta} \mid \cdot \sim \text{Normal}\Big([\bm{\Sigma}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}[\bm{X}'(\bm{z} - 0.5 \bm{1}_J - \bm{S}_{\beta}\text{\textbf{w}}) + \bm{\Sigma}_{\beta}^{-1}\bm{\mu}_{\beta}], [\bm{\Sigma}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}\Big), 
(\#eq:betaPostspPGOcc)
\end{equation}
...
\begin{equation}
\text{\textbf{w}} \mid \cdot \sim \text{Normal}\Big([\bm{S}_{\beta} + \bm{\Sigma}^{-1}]^{-1} [\bm{z} - 0.5\bm{1}_J - \bm{S}_{\beta}\bm{X}\bm{\beta}], [\bm{S}_{\beta} + \bm{\Sigma}^{-1}]^{-1}\Big).
(\#eq:wPostspPGOcc)
\end{equation}
...
\begin{equation}
\begin{bmatrix} \text{\textbf{w}} \\ \text{\textbf{w}}_0 \end{bmatrix} \mid \cdot \sim \text{Normal}\Bigg(\begin{bmatrix} \bm{0} \\ \bm{0} \end{bmatrix}, \begin{bmatrix} \bm{\Sigma}_{11} & \bm{\Sigma}_{12} \\ \bm{\Sigma}_{12}' & \bm{\Sigma}_{22} \end{bmatrix}\Bigg), 
(\#eq:mvn)
\end{equation}
...
\begin{equation}
\text{\textbf{w}}_0 \sim \text{Normal}(\bm{\Sigma}_{12}'\bm{\Sigma}_{11}^{-1}\text{\textbf{w}}, \bm{\Sigma}_{22} - \bm{\Sigma}_{12}'\bm{\Sigma}_{11}^{-1}\bm{\Sigma}_{12}).
(\#eq:wPredspPGOcc)
\end{equation}
...
1. Sample w$^{(q)}_{0, j}$ from Equation \@ref(eq:wPredspPGOcc), substituting in the current values at sample $q$ of the spatial parameters and latent spatial process at the observed locations. 
2. Compute the latent occurrence probability $\psi_j^{(q)}$ as $\text{logit}^{-1}(\bm{x}_{0, j}\bm{\beta}^{(q)} + \text{w}_{0, j}^{(q)})$.
...
The joint posterior distribution for the NNGP model is exactly the same as that of the full GP spatial occupancy model except the Gausian Process specification assigned to the latent spatial random effects \textbf{w} is replaced with an NNGP prior [@datta2016hierarchical].  
...
\begin{equation}
\text{\textbf{w}} \mid \cdot \sim \text{Normal}(\text{\textbf{B}}[\bm{S}_{\beta}^{-1}(\bm{\tilde{z}} - \bm{X}\bm{\beta})], \text{\textbf{B}})
\end{equation}

where $\bm{S}_{\beta}$ is a diagonal $J \times J$ matrix with diagonal entries equal to the latent Polya-Gamma variable values, $\tilde{z}_j = \frac{z_j - 0.5}{\omega_{j, \beta}}$, and \textbf{B} $= \bm{\tilde{\Sigma}}(\bm{s}, \bm{s}', \bm{\theta})^{-1} + \bm{S}_{\beta}$. $\bm{\tilde{\Sigma}}(\bm{s}, \bm{s}', \bm{\theta})$ is the sparse NNGP covariance matrix. For details of the NNGP covariance matrix, see @datta2016hierarchical and @finley2019efficient. 

As described by @finley2020spnngp and @datta2016hierarchical, the above block update of \textbf{w} is not computationally practical. Instead, we sequentially update the full conditionals individual for each $j = 1, \dots, J$ element of \textbf{w} following the algorithm in @datta2016hierarchical. This ensures each update of the full latent spatial random efffects vector occurs in $O(n)$ floating point operations (FLOPs). 
...
\begin{equation}
\text{\textbf{w}}_0 \mid \cdot \sim \text{Normal}(\text{\textbf{B}}^{-1}[\bm{S}_{\beta}^{-1}(\bm{\tilde{z}} - \bm{X}\bm{\beta})], \text{\textbf{B}}^{-1}). 
(\#eq:nngpPred)
\end{equation}
...
\begin{equation}
\bm{\beta}_i \sim \text{Normal}(\bm{\bar{\beta}}, \bm{T}_{\beta}),
(\#eq:msBeta)
\end{equation}

where $\bm{\bar{\beta}}$ is a vector of community level mean effects for each occurrence covariate effect (including the intercept) and $\bm{T}_{\beta}$ is a diagonal matrix with diagonal elements $\bm{\tau}_{\beta}$ that represent the variability of each occurrence covariate effect among species in the community.
...
\begin{equation}
\bm{\alpha}_i \sim \text{Normal}(\bm{\bar{\alpha}}, \bm{T}_{\alpha}),
(\#eq:msAlpha)
\end{equation}

where $\bm{\bar{\alpha}}$ is a vector of community level mean effects for each detection covariate effect (including the intercept) and $\bm{T}_{\alpha}$ is a diagonal matrix with diagonal elements $\bm{\tau}_{\alpha}$ that represent the variability of each detection covariate effect among species in the community.

We assign multivariate normal priors for the community level occurrence ($\bm{bar{\beta}}$) and detection ($\bm{\bar{\beta}}$) means, and assign indepdent inverse-Gamma priors on the community level occurrence ($\tau_{\beta}$) and detection ($\tau_{\alpha}$) variance parameters. Analogous to the single species occupancy model, we implement the model using Polya-Gamma data augmentation which induces fully Gibbs updates for all parameters. We specify Polya-Gamma data augmented variables for each species ($\omega_{i, j, \beta}$, $\omega_{i, j, k, \alpha}$, which follow the same scheme as that for the single species model in Equation \@ref(eq:polyaGamma), except all parameters in the equation are now indexed by $i$ for each species. The full joint posterior distribution thus takes the following form: 

$$
\begin{aligned}
\lbrack \bm{\alpha}, \bm{\beta}, \bm{z}, \bm{\omega}_{\beta}, \bm{\omega}_{\alpha}, \bm{\mu}_{\beta}, \bm{\mu}_{\alpha}, \bm{\tau}_{\beta}, \bm{\tau}_{\alpha} \mid \bm{y} \rbrack \propto \prod_{i = 1}^N \prod_{j = 1}^J \prod_{k = 1}^{K_j} &\text{Bernoulli}(y_{i, j, k} \mid p_{i, j, k}z_{i, j}) \times \\
& \text{Bernoulli}(z_{i, j} \mid \psi_{i, j}) \times \\
& \text{PG}(\omega_{i, j, \beta} \mid 1, 0) \times \\
& \text{PG}(\omega_{i, j, k, \alpha} \mid 1, 0) \times \\
& \text{Normal}(\bm{\beta} \mid \bm{\bar{\beta}}, \bm{T}_{\beta}) \times \\
& \text{Normal}(\bm{\alpha} \mid \bm{\bar{\alpha}}, \bm{T}_{\alpha}) \\
& \text{Normal}(\bm{\bar{\beta}} \mid \bm{\mu}_{\beta}, \bm{\Sigma}_{\beta}) \\
& \text{Normal}(\bm{\bar{\alpha}} \mid \bm{\mu}_{\alpha}, \bm{\Sigma}_{\alpha}) \\
& \prod_{r = 1}^{n_{\beta}} \text{IG}(\tau_{r, \beta} \mid a_{r, \beta}, b_{r, \beta}) \\
& \prod_{t = 1}^{n_{\alpha}} \text{IG}(\tau_{t, \alpha} \mid a_{t, \alpha}, b_{t, \alpha}),
\end{aligned}
$$
...
The Polya-Gamma data augmentation induces a Gibbs update for all parameters in the multispecies occupancy model. For each iteration, we first sample all community level parameters followed by species level parameters. We first sample the community level regression coefficients $\bm{\bar{\beta}}$ from 

\begin{equation}
\bm{\bar{\beta}} \mid \cdot \sim \text{Normal}([\bm{\Sigma}_{\beta}^{-1} + N\bm{T}_{\beta}^{-1}]^{-1}\Big[\sum_{i = 1}^N(\bm{T}_{\beta}\bm{\beta}_i) + \bm{\Sigma}_{\beta}^{-1}\bm{\mu}_{\beta}\Big], [\bm{\Sigma}_{\beta}^{-1} + N\bm{T}_{\beta}^{-1}]^{-1}).
(\#eq:betaBar)
\end{equation}

Similarly, we next sample the community level regression coefficients $\bm{\bar{\alpha}}$ from 

\begin{equation}
\bm{\bar{\alpha}} \mid \cdot \sim \text{Normal}([\bm{\Sigma}_{\alpha}^{-1} + N\bm{T}_{\alpha}^{-1}]^{-1}\Big[\sum_{i = 1}^N(\bm{T}_{\alpha}\bm{\alpha}_i) + \bm{\Sigma}_{\alpha}^{-1}\bm{\mu}_{\alpha}\Big], [\bm{\Sigma}_{\alpha}^{-1} + N\bm{T}_{\alpha}^{-1}]^{-1}).
(\#eq:alphaBar)
\end{equation}

Next, we sample the community level occurrence variance parameter for each regression coefficient, $\tau_{r, \beta}$, from the following inverse-Gamma full conditional:

\begin{equation}
\tau_{r, \beta} \mid \cdot \sim \text{IG}(a_{r, \beta} + \frac{N}{2}, a_{r, \beta} + \frac{\sum_{i = 1}^N(\beta_{i, r} - \bar{\beta}_r)^2}{2}).
(\#eq:tauBeta)
\end{equation}
...
\begin{equation}
\tau_{t, \alpha} \mid \cdot \sim \text{IG}(a_{t, \alpha} + \frac{N}{2}, a_{t, \alpha} + \frac{\sum_{i = 1}^N(\alpha_{i, t} - \bar{\alpha}_t)^2}{2}).
(\#eq:tauAlpha)
\end{equation}
...
\begin{equation}
\begin{split}
\omega_{i, j, \beta} \mid \cdot &\sim \text{PG}(1, \bm{x}_j'\bm{\beta}_i), \\
\omega_{i, j, k, \alpha} \mid \cdot &\sim \text{PG}(1, \bm{v}_{j, k}'\bm{\alpha}_i). 
\end{split}
(\#eq:omegaPostmsPGOcc)
\end{equation}
...
\begin{equation}
\bm{\beta}_i \mid \cdot \sim \text{Normal}\Big([\bm{T}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}[\bm{X}'(\bm{z}_i - 0.5 \bm{1}_J) + \bm{T}_{\beta}^{-1}\bm{\bar{\beta}}], [\bm{T}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}\Big), 
(\#eq:betaMsPGOcc)
\end{equation}

where $\bm{S}_{\beta}$ is a diagonal $J \times J$ matrix with diagonal etnries equal to the latent Polya-Gamma variable values for species $i$. Similarly, we sample the detection regression coefficients for species $i$ from 


\begin{equation}
\bm{\alpha}_i \mid \cdot \sim \text{Normal}\Big([\bm{T}_{\alpha}^{-1} + \bm{\tilde{V}}'\bm{S}_{\alpha}\bm{\tilde{V}}]^{-1}[\bm{\tilde{V}}'(\bm{\tilde{y}}_i - 0.5 \bm{1}_{J_i^*}) + \bm{T}_{\alpha}^{-1}\bm{\bar{\alpha}}], [\bm{T}_{\alpha}^{-1} + \bm{\tilde{V}}'\bm{S}_{\alpha}\bm{\tilde{V}}]^{-1}\Big).
(\#eq:alphaMsPGOcc)
\end{equation}
...
\begin{equation}
\begin{split}
&\text{logit}(\psi_{i, j}^{(q)}) = \bm{x}_{0, j}' \bm{\beta}_i^{(q)}, \\
...
\begin{equation}
\begin{split}
\omega_{i, j, \beta} \mid \cdot &\sim \text{PG}(1, \bm{x}_j'\bm{\beta}_i + \text{w}_{i, j}), \\
\omega_{i, j, k, \alpha} \mid \cdot &\sim \text{PG}(1, \bm{v}_{j, k}'\bm{\alpha}_i). 
\end{split}
...
\begin{equation}
\bm{\beta}_i \mid \cdot \sim \text{Normal}\Big([\bm{T}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}[\bm{X}'(\bm{z}_i - 0.5 \bm{1}_J - \bm{S}_{\beta}\text{\textbf{w}}_i) + \bm{T}_{\beta}^{-1}\bm{\bar{\beta}}], [\bm{T}_{\beta}^{-1} + \bm{X}'\bm{S}_{\beta}\bm{X}]^{-1}\Big),
(\#eq:betaspMsPGOcc)
\end{equation}

where $\bm{S}_{\beta}$ is a diagonal $J \times J$ matrix with diagonal etnries equal to the latent Polya-Gamma variable values for species $i$. The full conditioanl for the species-level detection regression coefficients is the same as in the non-spatial model shown in Equation \@ref(eq:alphaMsPGOcc). 

The spatial variance parameter for species $i$, $\sigma^2_i$, is sampled via a Gibbs update of the form 

\begin{equation}
\sigma^2_i \mid \cdot \sim \text{IG}\Big(\frac{J}{2} + a_{\sigma^2, i}, \frac{\text{\textbf{w}}_i'\bm{R}_i^{-1}\text{\textbf{w}}_i}{2} + b_{\sigma^2, i}\Big),
...
\begin{equation}
\text{\textbf{w}}_i \mid \cdot \sim \text{Normal}\Big([\bm{S}_{\beta} + \bm{\Sigma}_i^{-1}]^{-1} [\bm{z}_i - 0.5\bm{1}_J - \bm{S}_{\beta}\bm{X}\bm{\beta}_i], [\bm{S}_{\beta} + \bm{\Sigma}_i^{-1}]^{-1}\Big).
(\#eq:wPostspMsPGOcc)
\end{equation}
...
The joint posterior distribtuion for the multispecies NNGP occupancy model is exactly the same as that of the full GP multispecies model except the GP prior assigned to the latent spatial random effects \textbf{w}$_i$ for each species is replaced with an NNGP prior [@datta2016hierarchical]. 

### MCMC sampler and prediction

The full conditionals for all variables except the spatial variance parameters $\sigma^2_i$ and the latent spatial process \textbf{w}$_i$ follow the same full conditionals as described in the full GP multispecies spatial occupancy model. The full conditionals for $\sigma^2_i$ and \textbf{w}$_i$ follow exactly those described for the single species NNGP model in Section \@ref(samplerspPGOccNNGP), where all parameters are now indexed by each species $i$. Prediction is also exactly analogous to that described for the single species NNGP model in Section \@ref(predictionspPGOccNNGP).
...
\begin{split}
\omega_{j, \beta} \mid \cdot &\sim \text{PG}(1, \bm{x}_j'\bm{\beta}), \\
\omega_{r, j, k, \alpha} \mid \cdot &\sim \text{PG}(1, \bm{v}_{r, j, k}'\bm{\alpha}_r), 
\end{split}
