...
The `PGOcc` function fits single species occupancy models using $\pg$ latent variables, which makes it more efficient than standard implementations of occupancy models using a logit link function [@clark2019; @polson2013]. `PGOcc` has the following arguments: 
...
The last three arguments (`k.fold`, `k.fold.threads`, and `k.fold.seed`) provide functionality for performing k-fold cross-validation. These three arguments are in all `spOccupancy` model fitting functions. In this vignette we won't perform cross-validation and will leave these arguments alone (see the separate cross-validation vignette for details on performing k-fold cross-validation). 
