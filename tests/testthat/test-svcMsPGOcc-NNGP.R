...
test_that("out is of class svcMsPGOcc", {
  expect_s3_class(out, "svcMsPGOcc")
})
...
test_that("verbose prints to the screen", {
  ...
  expect_output(svcMsPGOcc(occ.formula = occ.formula, 
                 det.formula = det.formula, 
                 data = data.list,
                 inits = inits.list, 
                 n.batch = n.batch, 
...
# Check waicOcc -----------------------
test_that("waicOCC works for svcMsPGOcc", {
...
test_that("fitted works for svcMsPGOcc", {
...
test_that("predict works for svcMsPGOcc", {
...
p.svc <- length(svc.cols)
n.factors <- 3
phi <- runif(n.factors * p.svc, 3 / .9, 3 / .1)
nu <- rep(1, n.factors * p.svc)
...
n.factors <- 3
phi <- rep(3 / .7, n.factors * p.svc)
sigma.sq <- rep(2, n.factors * p.svc)
nu <- rep(2, n.factors * p.svc)

dat <- simMsOcc(J.x = J.x, J.y = J.y, n.rep = n.rep, N = N, beta = beta, alpha = alpha,
	        psi.RE = psi.RE, p.RE = p.RE, sp = TRUE, sigma.sq = sigma.sq, 
		phi = phi, nu = nu, cov.model = 'matern', svc.cols = svc.cols,
                factor.model = TRUE, n.factors = n.factors)
...
# Community-level covariate effects
# Occurrence
beta.mean <- c(0.2, 0.5, 1.2, -0.2)
p.occ <- length(beta.mean)
tau.sq.beta <- c(0.6, 1.5, 2.3, 0.5)
# Detection
alpha.mean <- c(0, -0.5)
tau.sq.alpha <- c(1, 2.3)
p.det <- length(alpha.mean)
...
test_that("predict works for svcMsPGOcc", {
  n.post.samples <- out$n.post * out$n.chains
  X.0 <- X.0
  colnames(X.0) <- c('int', 'occ.cov.1', 'occ.cov.2', 'occ.cov.3')
  pred.out <- predict(out, X.0, coords.0, verbose = FALSE)
  expect_type(pred.out, "list")
  expect_equal(dim(pred.out$psi.0.samples), c(n.post.samples, N, nrow(X.0)))
  expect_equal(dim(pred.out$z.0.samples), c(n.post.samples, N, nrow(X.0)))
...
dat <- simMsOcc(J.x = J.x, J.y = J.y, n.rep = n.rep, N = N, beta = beta, alpha = alpha,
	        psi.RE = psi.RE, p.RE = p.RE, sp = TRUE, sigma.sq = sigma.sq,
		phi = phi, nu = nu, cov.model = 'matern',
                factor.model = TRUE, n.factors = n.factors, n.rep.max = n.rep.max)
